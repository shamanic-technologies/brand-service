...
/**
 * Migration: Update bulk_insert_web_pages - URLs in list = should_scrape=TRUE
 * 
 * LOGIC:
 * - URLs in the LLM output → should_scrape = TRUE (always, they were selected)
 * - Other URLs in the same domain → should_scrape = FALSE
 * 
 * The JSON schema no longer contains should_scrape field.
 * If the LLM includes a URL, it means it should be scraped.
 */
...
exports.down = (pgm) => {
  pgm.sql(`
    -- Just drop, previous migration will restore
    DROP FUNCTION IF EXISTS bulk_insert_web_pages(jsonb);
  `);
};
