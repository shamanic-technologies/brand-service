...
exports.up = (pgm) => {
  pgm.sql(`
    -- ========================================
    -- 1. DROP COLUMNS FROM web_pages
    -- ========================================
    ALTER TABLE web_pages
      DROP COLUMN IF EXISTS is_informative,
      DROP COLUMN IF EXISTS relevance_rationale;


    -- ========================================
    -- 2. UPDATE bulk_insert_web_pages FUNCTION
    -- ========================================
    ...
    COMMENT ON FUNCTION bulk_insert_web_pages IS 'Bulk inserts web pages from LLM output with URL normalization. Accepts domain and LLM JSON array with url, page_category, and should_scrape. Deduplicates based on normalized_url. Auto-creates web_domain if needed.';
  `);
};
...
exports.down = (pgm) => {
  pgm.sql(`
    -- Restore columns
    ALTER TABLE web_pages
      ADD COLUMN is_informative BOOLEAN DEFAULT true,
