...
exports.up = (pgm) => {
  pgm.sql(`
    -- Drop old text-based function
    DROP FUNCTION IF EXISTS bulk_insert_web_pages(text);

    -- Create new function that accepts JSONB directly (no stringify needed)
    ...
    COMMENT ON FUNCTION bulk_insert_web_pages IS 'Bulk inserts web pages from LLM output. Accepts JSONB directly (array or object). No JSON.stringify needed in N8N! Supports: direct array, db_ready_output field, Gemini API response, or pages field.';
  `);
};
