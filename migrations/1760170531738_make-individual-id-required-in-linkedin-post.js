 ...
 */
export const up = (pgm) => {
  // Drop the old function with optional individual_id if it exists
  pgm.dropFunction('upsert_individual_linkedin_post', [
    { name: 'p_individual_id', type: 'uuid' },
    { name: 'p_raw_data', type: 'jsonb' },
  ], { ifExists: true });

  // Create the new function with required individual_id (no default)
  pgm.createFunction(
...
export const down = (pgm) => {
  // Drop the function with required individual_id
  pgm.dropFunction('upsert_individual_linkedin_post', [
    { name: 'p_individual_id', type: 'uuid' },
    { name: 'p_raw_data', type: 'jsonb' },
  ]);

  // Restore the old function with optional individual_id
  pgm.createFunction(
    'upsert_individual_linkedin_post',
    [
