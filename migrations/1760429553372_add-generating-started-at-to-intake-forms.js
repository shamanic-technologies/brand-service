 ...
 */
export const up = (pgm) => {
  // Add generating_started_at column to intake_forms
  pgm.addColumn('intake_forms', {
  ...
  // Create function with new return type including generating_started_at
  pgm.createFunction(
    'upsert_intake_form',
    [
    ...
    DECLARE
      ...
      v_intake_data jsonb;
      v_org_generating_started_at timestamptz;
    BEGIN
      -- Get the organization ID and its generating_started_at from external_organization_id
      SELECT organizations.id, organizations.generating_started_at
      INTO v_organization_id, v_org_generating_started_at
      FROM organizations
      ...
      -- Reset organization status to NULL
  ...
  pgm.sql(`
    COMMENT ON FUNCTION upsert_intake_form IS 'Upserts intake form data for an organization. Takes JSONB with form fields and external_organization_id. Handles both single objects and arrays. Automatically updates last_synced_at timestamp and resets organization status to NULL.';
  `);
};
...
export const down = (pgm) => {
  // Drop the column
  pgm.dropColumn('intake_forms', 'generating_started_at');
  
  // Note: Function rollback handled by previous migration
};
