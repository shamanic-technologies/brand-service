...
exports.up = (pgm) => {
  pgm.sql(`
    ...
    -- Create updated function that can handle both array and Gemini response structure
    ...
    BEGIN
      ...
      -- 2. Extract pages array from different possible structures
      ...
      ELSIF v_parsed_input ? 'candidates' THEN
        ...
        -- Extract from: candidates[0].content.parts[0].text (which contains JSON string)
        DECLARE
          v_text_content TEXT;
        BEGIN
    ...
    COMMENT ON FUNCTION bulk_insert_web_pages IS 'Bulk inserts web pages from LLM output. Accepts: direct JSON array, Gemini API response, or object with pages/db_ready_output field. Auto-extracts array from structure. Domain is auto-computed from URL. Deduplicates based on normalized_url.';
  `);
};
