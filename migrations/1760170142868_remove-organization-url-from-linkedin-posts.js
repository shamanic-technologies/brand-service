 ...
 */
export const up = (pgm) => {
  // Drop the old function with 3 parameters if it exists
  ...
  // Create the new function with only 2 parameters (without organization_url)
  pgm.createFunction(
    'upsert_individual_linkedin_post',
    [
      { name: 'p_individual_id', type: 'uuid', mode: 'IN', default: null },
    ...
    BEGIN
      ...
      DO UPDATE SET
        individual_id = COALESCE(EXCLUDED.individual_id, individuals_linkedin_posts.individual_id),
  ...
  // Drop the index on organization_url
  pgm.dropIndex('individuals_linkedin_posts', 'organization_url');

  // Drop the organization_url column
  pgm.dropColumn('individuals_linkedin_posts', 'organization_url');
};
...
export const down = (pgm) => {
  // Add back the organization_url column
  pgm.addColumn('individuals_linkedin_posts', {
    organization_url: {
      type: 'text',
      notNull: false,
  ...
  // Drop the function with 2 parameters
  pgm.dropFunction('upsert_individual_linkedin_post', [
    { name: 'p_individual_id', type: 'uuid' },
    { name: 'p_raw_data', type: 'jsonb' },
  ]);

  // Restore the old function with 3 parameters
  pgm.createFunction(
    'upsert_individual_linkedin_post',
    [
      { name: 'p_individual_id', type: 'uuid', mode: 'IN', default: null },
    ...
    BEGIN
      ...
      DO UPDATE SET
        individual_id = COALESCE(EXCLUDED.individual_id, individuals_linkedin_posts.individual_id),
        organization_url = COALESCE(EXCLUDED.organization_url, individuals_linkedin_posts.organization_url),
        raw_data = EXCLUDED.raw_data,
        post_type = EXCLUDED.post_type,
        linkedin_url = EXCLUDED.linkedin_url,
        content = EXCLUDED.content,
