...
export const up = (pgm) => {
  ...
  // Recreate the function without organization_url parameter
  pgm.createFunction(
  ...
  // Update comment
  pgm.sql(`
    COMMENT ON FUNCTION upsert_organization_linkedin_post IS 'Upserts an organization LinkedIn post from Apify scraper data. Takes organization_id (optional) and raw_data (required).';
  `);

  // Drop the index on organization_url
  pgm.dropIndex('organizations_linkedin_posts', 'organization_url');

  // Remove the organization_url column
  pgm.dropColumn('organizations_linkedin_posts', 'organization_url');
};
...
export const down = (pgm) => {
  ...
  pgm.addColumn('organizations_linkedin_posts', {
    organization_url: {
      type: 'text',
      notNull: false,
  ...
  // Restore the old function with organization_url parameter
  pgm.createFunction(
    'upsert_organization_linkedin_post',
    [
      { name: 'p_organization_id', type: 'uuid', mode: 'IN', default: null },
    ...
    BEGIN
      ...
      DO UPDATE SET
        organization_id = COALESCE(EXCLUDED.organization_id, organizations_linkedin_posts.organization_id),
        organization_url = COALESCE(EXCLUDED.organization_url, organizations_linkedin_posts.organization_url),
        raw_data = EXCLUDED.raw_data,
        post_type = EXCLUDED.post_type,
        linkedin_url = EXCLUDED.linkedin_url,
        content = EXCLUDED.content,
