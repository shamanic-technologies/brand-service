/**
 * Migration: Fix scraped_url_firecrawl join to use source_url instead of url
 * 
 * Updates v_individuals_linkedin_articles and v_organization_linkedin_articles
 * to join on scraped.source_url (original URL) instead of scraped.url (final URL after redirects)
 */

exports.up = (pgm) => {
  // Drop and recreate v_individuals_linkedin_articles with correct join
  pgm.sql(`
  ...
  pgm.sql(`
    ...
    SELECT
      ...
      scraped.source_url AS scraped_source_url,
    ...
    LEFT JOIN
      scraped_url_firecrawl scraped ON lp.article_link = scraped.source_url
    WHERE
      lp.has_article = true
    ORDER BY
      o.external_organization_id,
  ...
  // Drop and recreate v_organization_linkedin_articles with correct join
  pgm.sql(`
...
exports.down = (pgm) => {
  // Revert to joining on scraped.url
  pgm.sql(`
    DROP VIEW IF EXISTS v_individuals_linkedin_articles;
  `);
