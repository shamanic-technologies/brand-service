/**
 * Migration: Fix bulk_upsert_organization_relations to keep subdomains
 * 
 * BUG: extract_domain_from_url strips ALL subdomains, so:
 *   - https://ramilabs.com/ → ramilabs.com
 *   - https://rpgen.ramilabs.com/ → ramilabs.com (WRONG! should be rpgen.ramilabs.com)
 * 
 * This causes organizations with different subdomains to be treated as the same org
 * and overwrite each other via ON CONFLICT (domain).
 * 
 * FIX: Create extract_hostname_from_url that:
 *   - Removes protocol (http://, https://)
 *   - Removes www. prefix ONLY
 *   - Removes path, query string, port
 *   - KEEPS other subdomains (rpgen., app., api., etc.)
 * 
 * Then update bulk_upsert_organization_relations to use this function.
 */

exports.shorthands = undefined;

exports.up = (pgm) => {
  // 1. Create new function that keeps subdomains (except www)
  pgm.sql(`
    CREATE OR REPLACE FUNCTION extract_hostname_from_url(url_input text)
    RETURNS text AS $$
    DECLARE
      hostname_result text;
    BEGIN
      ...
      hostname_result := LOWER(TRIM(url_input));

      -- Remove protocol (http://, https://, etc.)
      hostname_result := regexp_replace(hostname_result, '^https?://', '', 'i');

      -- Remove www. prefix ONLY (not other subdomains)
      hostname_result := regexp_replace(hostname_result, '^www\\.', '', 'i');

      -- Remove everything after first slash (paths)
      hostname_result := regexp_replace(hostname_result, '/.*$', '');

      -- Remove query strings (everything after ?)
      hostname_result := regexp_replace(hostname_result, '\\?.*$', '');

      -- Remove port number
      hostname_result := regexp_replace(hostname_result, ':[0-9]+$', '');

      -- DO NOT strip subdomains - keep rpgen.ramilabs.com as-is
      
      RETURN hostname_result;
    END;
    $$ LANGUAGE plpgsql IMMUTABLE;

    COMMENT ON FUNCTION extract_hostname_from_url IS 
      'Extracts hostname from URL, keeping subdomains (except www). '
      'Use this for organization URLs where subdomains represent different entities. '
      'Example: https://rpgen.ramilabs.com/ -> rpgen.ramilabs.com';
  `);

  // 2. Update bulk_upsert_organization_relations to use extract_hostname_from_url
  ...
  pgm.sql(`
    ...
    BEGIN
      ...
      LOOP
        -- Extract FULL hostname from URL (keeps subdomains like rpgen.ramilabs.com)
        v_target_domain := extract_hostname_from_url(v_relation_record->>'organization_url');
...
exports.down = (pgm) => {
  ...
  pgm.sql(`DROP FUNCTION IF EXISTS extract_hostname_from_url(text);`);
  
  // Revert bulk_upsert_organization_relations to use extract_domain_from_url
  pgm.sql(`DROP FUNCTION IF EXISTS bulk_upsert_organization_relations(text, text);`);
