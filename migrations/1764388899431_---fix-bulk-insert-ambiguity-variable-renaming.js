...
exports.up = (pgm) => {
  pgm.sql(`
    ...
    DECLARE
      ...
      v_page_id UUID;
      v_existing_id UUID;
      v_norm_url TEXT; -- Renamed to avoid ambiguity
    BEGIN
      ...
      LOOP
        -- Normalize URL upfront
        v_norm_url := normalize_url(v_page_record->>'url');
        
        -- Check if exists (explicit table alias)
...
exports.down = (pgm) => {
  // No down necessary as it replaces the same function signature
};
