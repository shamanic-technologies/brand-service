/**
 * Migration: Update bulk_upsert_organization_thesis to use clerk_organization_id
 * 
 * Changes p_external_organization_id to p_clerk_organization_id for consistency
 * with the rule that all inter-service communication must use clerkOrgId.
 */
...
exports.up = (pgm) => {
  ...
  // Create the new function using clerk_organization_id
  pgm.createFunction(
    'bulk_upsert_organization_thesis',
    [
      { name: 'p_theses_data', type: 'jsonb', mode: 'IN' },
      { name: 'p_clerk_organization_id', type: 'text', mode: 'IN' }
    ],
    {
    ...
    BEGIN
      -- 1. Find organization by clerk_organization_id
      SELECT id INTO v_organization_id
      FROM organizations
      WHERE clerk_organization_id = p_clerk_organization_id;

      IF v_organization_id IS NULL THEN
        RAISE EXCEPTION 'Organization not found with clerk_organization_id: %', p_clerk_organization_id;
      END IF;
      ...
      LOOP
        ...
        IF NOT EXISTS (
          ...
          RAISE WARNING 'Thesis ID % not found for organization %, skipping', v_thesis_id, p_clerk_organization_id;
          CONTINUE;
        END IF;
  ...
  pgm.sql(`
    COMMENT ON FUNCTION bulk_upsert_organization_thesis IS 
      ...
      Uses clerk_organization_id for organization lookup (preferred over external_organization_id).
...
exports.down = (pgm) => {
  ...
  pgm.dropFunction('bulk_upsert_organization_thesis', [
    { name: 'p_theses_data', type: 'jsonb' },
    { name: 'p_clerk_organization_id', type: 'text' }
  ]);
