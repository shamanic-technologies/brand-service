 ...
 */
export const up = (pgm) => {
  // Drop and recreate the function to update intake_forms.status instead of organizations.status
  ...
  pgm.sql(`
    COMMENT ON FUNCTION upsert_intake_form IS 'Upserts intake form data for an organization. Takes JSONB with form fields and external_organization_id. Handles both single objects and arrays. Automatically updates last_synced_at timestamp and resets intake_forms.status to NULL.';
  `);
};
...
export const down = (pgm) => {
  // Rollback handled by previous migration
};
