...
export const up = (pgm) => {
  ...
  pgm.createFunction(
    ...
    {
      returns: 'TABLE(target_org_id uuid, target_org_external_id text, target_org_name text, target_org_url text, target_org_linkedin_url text, target_org_domain text, relation_type organization_relation_type, relation_confidence_level text, relation_confidence_rationale text, relation_status organization_relation_status, relation_created_at timestamptz, relation_updated_at timestamptz)',
      language: 'sql',
      replace: true,
    },
    `
  ...
  // Update comment
  pgm.sql(`
    COMMENT ON FUNCTION get_target_organizations IS 'Returns all target organizations related to a source organization identified by external_organization_id. Returns target organization basic info, relation details including type (enum) and status.';
  `);
};
