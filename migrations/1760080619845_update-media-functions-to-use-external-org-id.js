 ...
 */
export const up = (pgm) => {
  // Drop existing functions that use internal organization_id
  ...
  // Create new functions that accept external_organization_id (Clerk ID)
  pgm.createFunction(
    ...
    `
      ...
      INNER JOIN
        media_assets AS ma ON ma.organization_id = org.id
      LEFT JOIN
        supabase_storage AS ss ON ma.supabase_storage_id = ss.id
      WHERE
...
export const down = (pgm) => {
  // Drop new functions with external_organization_id
  pgm.dropFunction('get_shareable_media_assets_by_organization_id', [
    { name: 'p_external_organization_id', type: 'text', mode: 'IN' },
  ]);
  ...
  // Restore old functions with internal organization_id
  pgm.createFunction(
    'get_shareable_media_assets_by_organization_id',
    [{ name: 'p_organization_id', type: 'uuid', mode: 'IN' }],
    {
