 ...
 */
export const up = (pgm) => {
  // First, make raw_response nullable in the table
  pgm.alterColumn('scraped_url_firecrawl', 'raw_response', {
    notNull: false,
  });
  ...
  // Recreate the function with optional raw_response parameter
  pgm.createFunction(
    ...
    BEGIN
      ...
      -- Upsert the Firecrawl scraped data with full response
  ...
  // Add updated comment
  pgm.sql(`
    COMMENT ON FUNCTION upsert_scraped_url_firecrawl IS 'Upserts a scraped URL into the Firecrawl table. The raw_response parameter is optional - if NULL, only the URL is stored for later scraping. If provided, all scraped data is stored.';
  `);
};
...
export const down = (pgm) => {
  ...
  // Restore the old function (requires raw_response)
  pgm.createFunction(
    'upsert_scraped_url_firecrawl',
    [
      { name: 'p_url', type: 'text', mode: 'IN' },
  ...
  // Make raw_response NOT NULL again
  pgm.alterColumn('scraped_url_firecrawl', 'raw_response', {
    notNull: true,
  });
};
