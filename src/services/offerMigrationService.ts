...
const NAMING_SYSTEM_PROMPT =
  ...
  `Answer with a name of AT MOST ${GENERATED_OFFER_NAME_MAX_WORDS} words and AT MOST ${GENERATED_OFFER_NAME_MAX_CHARS} ` +
...
const NAMING_RESPONSE_SCHEMA = {
  ...
  properties: {
    name: {
      type: 'string',
      description: `The offer name. At most ${GENERATED_OFFER_NAME_MAX_WORDS} words, at most ${GENERATED_OFFER_NAME_MAX_CHARS} characters. Empty when the input says too little.`,
    },
  },
  required: ['name'],
} as const;
...
async function askForOfferName(
  ...
  // The GENERATED rule, not the supplied one: this name is the model's, not a
  // person's, so it is held to the tighter 2-word / 20-character limit the
  // naming prompt above asks for. Reading the supplied rule here would let the
  // migration mint a sixty-character sentence onto a row four services key
  // their display on.
  const problem = generatedOfferNameProblem(name);
