...
| PUT | `/orgs/brands/:brandId/click-destination` | Set the per-brand page outreach clicks should land on (default = brand domain, user-overridable). Body `clickDestinationUrl` must be a valid http(s) URL (normalized; non-http(s)/malformed/localhost/IP/missing-TLD → 400, fail loud). Brand-scoped config, NOT brand global identity; 403 if brand not in caller's org. Idempotent; returns `{ clickDestinationUrl }`. The saved value surfaces as `clickDestinationUrl` (string \| null) on the brand read responses. |
...
- `brand_click_destination` — one row per brand (PK = `brand_id`): the page outreach clicks should land on (`click_destination_url`, http(s)). Brand-scoped config, NOT brand global identity; unset = no row (read returns `null`). FK → brands ON DELETE CASCADE
