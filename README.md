...
| GET | `/orgs/brands/:brandId/sales-rep-phone` | The one number to ring when a sales interest lands on this brand, or `null` when the brand never stated one ("nobody to ring" — a first-class answer, not an error). Per-brand config keyed on `(org, brand)`; 403 if brand not in caller's org. `200 { salesRepPhone }` |
| PUT | `/orgs/brands/:brandId/sales-rep-phone` | State (or change) the number to ring when a prospect replies interested — the rep on it is phoned within the minute. BRAND grain (one number covers every offer, funnel and channel), per-brand config mirroring click-destination scoping. Body `{ salesRepPhone }` accepts any typed format carrying a country code (`+` or `00` prefix, 8-15 digits) and stores strict E.164; a national number with no country code, or anything unparseable, → 400 (nothing is inferred — the value goes straight to a telephony provider). Idempotent upsert; 403 if brand not in caller's org. Read back here or via the `salesRepPhone` field on the INTERNAL brand read. `200 { salesRepPhone }` |
| DELETE | `/orgs/brands/:brandId/sales-rep-phone` | Remove the number: the row is deleted and the brand goes back to "nobody to ring". Idempotent — removing one that was never stated is a 200, not a 404. `200 { salesRepPhone: null }` |
...
- `brand_sales_rep_phones` — one row per `(org_id, brand_id)`: the one number to ring when a sales interest lands on the brand (`phone` text NOT NULL, strict E.164 — row presence = set). BRAND grain deliberately, not campaign grain: a campaign is (offer x funnel x channel), so a brand running four channels on one offer would retype one fact four times, and a brand with no campaign yet could state nothing at all. Per-brand config (mirrors `brand_click_destinations` scoping); an unset brand has no row and the INTERNAL brand read returns `salesRepPhone: null` — never an empty string, never defaulted or inferred from the user row's phone or the WhatsApp link. Served on the internal brand reads only (org-scoped by `x-org-id`; where no org is named, it answers only when a single org has stated a number), and absent from the public brand read and the share-token resolve
