...
| GET | `/orgs/brands/:brandId/sales-economics` | Read brand sales economics: conversion metrics incl. decimal self-serve sub-rates `visitToSignupPct` + `signupToPaidClientPct`, plus DERIVED `visitToClosePct` = visitToSignupPct·signupToPaidClientPct/100, + `businessModel` + `funnelStages` (always array, `[]` unset) + `optimizationGoal` (always value, `"sales"` unset) (`{ salesEconomics: null }` when unset; 403 if brand not in caller's org) |
...
- `brand_click_destinations` — one row per brand: the chosen click-destination URL (`click_destination_url` text, NOT NULL — row presence = set). Per-brand config (mirrors `brand_sales_economics` scoping), reused across the brand's campaigns; unset brand has no row and reads `clickDestinationUrl: null`
