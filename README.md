...
| PUT | `/orgs/brands/:brandId/current-goal` | Update the brand-owned current runtime goal (`currentGoal: signup\|meetingBooked\|purchase\|websiteVisit\|positiveReply\|whatsappConversation\|combinedSales`) without editing campaigns. This is the canonical goal vocabulary campaign-service passes to features-service runtime candidate selection, and it is the **single authority** on what a brand optimizes for. The sales-economics `optimizationGoal` is NOT a second goal field: it is derived from this one on every read, and the stored `optimization_goal` column only records the raw wire spelling so two spellings sharing one runtime goal stay distinguishable. |
...
| GET | `/orgs/brands/:brandId/sales-funnels` | What this brand has **said** about the funnels it sells through — `{ declared, funnels }`. A funnel is one chain from the first signal outreach can buy down to a paid client (`reply_meeting`, `visit_meeting`, `visit_signup`, `visit_form`) and owns its OWN `rates` (only the legs of its chain), `lifetimeRevenueUsd`, `destinationUrl` and `bookingUrl`, plus its `goal` (brand-service wire) + `currentGoal` (runtime token). A value the brand never declared is `null`, never 0. **Read `declared` before `funnels`**: `declared: true` + `[]` = the brand stated it sells through NONE (a real answer — it is unrankable, say so); `declared: false` + `[]` = it has never said anything (a gap — surface it, never render it as "sells through nothing"). The set is only ever stated, never derived from `brand_sales_economics` (every rate there has a server default, so absence signals nothing). 403 if brand not in caller's org |
| PUT | `/orgs/brands/:brandId/sales-funnels` | State the **whole set**: exactly these funnels, no others. Body `{ funnelKeys: [...] }`. Funnels already in the set keep their economics (restating never wipes them); funnels dropped from it lose their declaration and economics together. `{ "funnelKeys": [] }` is legal and is the ONLY way a brand states it sells through **nothing** — a different answer from never having said anything. Validated whole before anything is written, so a bad member rejects the set 400 and nothing is half-applied. `200 { declared, funnels }` |
...
| DELETE | `/orgs/brands/:brandId/sales-funnels/:funnelKey` | The brand no longer sells through this funnel. Removes the declaration **and its economics** together, so nothing is left behind for a consumer to rank on. Idempotent (undeclaring an undeclared funnel is a 200). Does NOT un-state the set: removing the LAST funnel keeps `declared: true`, because the brand has stated it sells through none. `200 { declared, funnels }` |
...
| GET | `/internal/brands/:brandId/sales-funnels` | Service-auth read of the funnels a brand **authorizes**, keyed by brandId with NO org context — what campaign-service arbitration ranks over. Same shape as the org read: each funnel carries its `goal` + `currentGoal` and its own rates / lifetime revenue / destinations. **Read `declared` before `funnels`** — `declared: true` + `[]` is the brand stating it sells through none (report it unrankable), `declared: false` is a producer gap (surface it). Do NOT substitute a plausible set, and do NOT derive one from the stored economics. A brand id we hold nothing for is a third answer and is rejected 404, never served as `declared: false`. |
...
- `brands` — global silver brand identity keyed by normalized domain, with `current_goal` (`signup`/`meetingBooked`/`purchase`/`websiteVisit`/`positiveReply`/`whatsappConversation`/`combinedSales`) as the brand-owned runtime goal — the **single authority** on what a brand optimizes for (see `brand_sales_economics.optimization_goal`). `url` and `domain` are NULLABLE: a no-website brand (created via `POST /orgs/brands { name }`) has neither and is identified by its user-provided `name`; it extracts fields from its pasted business context instead of a scrape
...
- `brand_sales_economics` — one row per brand: 5 sales conversion-economics metrics (lifetime revenue + 4 funnel rates) + single-step rates `visit_to_paid_client_pct` (default 5) / `reply_to_paid_client_pct` (default 25) for the `website_visits` / `positive_replies` goals + two-step rates `visit_to_form_submission_pct` (default 25) / `form_submission_to_paid_client_pct` (default 20) — NOT NULL, always served (incl. the effective layer) for the `form_submissions` goal + `business_model` (`b2c`/`b2b`, nullable) + `funnel_stages` (jsonb, default `[]`) + `optimization_goal` (text, default `sales`) — NOT a second goal field: `brands.current_goal` is the only authority and every read derives the goal from it, while this column records the raw wire SPELLING the caller sent so two spellings that share one runtime goal stay distinguishable (`form_submissions` under `signup`, `website_purchase` under `purchase` — both recovered on the org read, collapsed to the runtime-safe base on the internal read). Reused across sales campaigns
- `brand_sales_funnels` — one row per `(brand_id, funnel_key)` (`reply_meeting`/`visit_meeting`/`visit_signup`/`visit_form`, CHECK-constrained): the funnels a brand DECLARED it sells through. The row's **presence is the declaration**, and every value column is NULLABLE with **no server default** — `null` means the brand never gave us that number, never 0. Each funnel carries its own 8 possible rate columns (only the legs of its chain are filled), `lifetime_revenue_usd`, `destination_url` and `booking_url`. `meeting_booked_to_attended_pct` (the meeting show-up rate) and `booking_url` exist ONLY here. This is the layer campaign-service arbitration reads; `brand_sales_economics` stays the brand-wide legacy layer its current consumers still read
- `brand_sales_funnel_declarations` — one row per brand (PK = brand_id), presence = **the brand has stated which funnels it sells through**. Exists because zero funnel rows is ambiguous on its own: "stated it sells through none" and "never told us anything" are different answers with opposite consumer behaviour. Declaring any funnel states the set; undeclaring one does NOT un-state it
