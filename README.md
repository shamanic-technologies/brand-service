...
| `pnpm migrate:offers` | ONE-TIME move of brand-scoped config (confirmed value-proposition fields + declared sales funnels) onto an OFFER, one per (org, brand). Names it with an LLM through chat-service, declares a runs-service PLATFORM run, idempotent / `--dry-run` / reversible by `migrated_from_brand_at` |
...
| POST | `/orgs/brands/:brandId/offers` | Create an OFFER — a distinct thing the org sells under this brand, with its OWN value proposition, its OWN declared sales funnels and its OWN economics. Body `{ name }`: at most 2 words, at most 20 characters, unique within the brand case-insensitively (409 on a duplicate, 400 on a bad shape). There is no primary offer — several run at once and none outranks another. `201 { offer }` |
| GET | `/orgs/brands/:brandId/offers` | Every offer this org sells under this brand, OLDEST FIRST. `200 { offers }` |
| GET | `/orgs/offers/:offerId` | Read one offer. 403 if it belongs to another org, 404 if unknown |
| PATCH | `/orgs/offers/:offerId` | Rename an offer (`{ name }`, same shape rules and same 409). `200 { offer }` |
| GET | `/orgs/offers/:offerId/user-fields` | The offer's 7 user-facing fields, same shape as the brand-scoped read. The CONFIRMED layer is per offer — what THIS offer promises. The SUGGESTED layer stays brand-scoped: a suggestion is what the extractor read off the brand's site, and the site says one thing whichever offer is described |
| PUT | `/orgs/offers/:offerId/user-fields` | Confirm (upsert) the offer's user-facing fields. Same 400 on an unknown key, same response shape |
| GET | `/orgs/offers/:offerId/sales-funnels` | The funnels this OFFER sells through and what each is worth — identical payload to the brand-scoped read, scoped to one offer |
| PUT | `/orgs/offers/:offerId/sales-funnels` | State the whole set for this offer. Same rules as the brand-scoped route (empty set refused 400; dropped funnels switched off with their numbers kept) |
| PUT | `/orgs/offers/:offerId/sales-funnels/:funnelKey` | Declare one funnel on this offer and price it. Same partial semantics and same 400s as the brand-scoped route |
| DELETE | `/orgs/offers/:offerId/sales-funnels/:funnelKey` | Switch a funnel off on this offer (`?erase=true` to forget it). Same last-active refusal, scoped to the offer |
| GET | `/orgs/brands/:brandId/user-fields` | **Resolves to the brand's EARLIEST offer** (transitional — unchanged payload, and it does not move when a second offer is added). The 7 user-facing "confirmed" fields: `{ fields: { <key>: { value, provenance } } }` for services, dreamOutcome, perceivedLikelihood, socialProof, riskReversal, urgency, scarcity. `confirmed` = user-validated value; `suggested` = most-recent NON-EXPIRED auto-extract prefill (or null). Does NOT trigger extraction. 403 if brand not in caller's org |
| PUT | `/orgs/brands/:brandId/user-fields` | **Writes onto the brand's EARLIEST offer** (transitional; one is created from the brand's own name if the brand has none). Confirm (upsert) user-facing fields, DURABLE (no TTL). Body `{ fields: Record<key, value> }`; each key MUST be one of the 7 user-facing keys or the request is rejected 400 (nothing written). Returns the updated view in the same shape as GET. `200 { fields }` |
...
| GET | `/internal/brands/:brandId/offers` | Service-auth read of a brand's OFFERS, oldest first. The org comes from `x-org-id`, else the single claiming org, else 400 `ORG_REQUIRED`; a brand no org claims answers `{ offers: [] }` (unset, never a 404) |
| GET | `/internal/offers/:offerId/sales-funnels` | Service-auth read of the funnels ONE OFFER authorizes. **ACTIVE only** — a scheduler must never rank a funnel the org switched off. The offer id already names one org, so no org resolution is needed; an unknown offer is a 404 |
...
- `brand_user_fields` — the user-facing "confirmed" layer, now per OFFER: one row per `(org_id, brand_id, offer_id, field_key)` (UNIQUE **NULLS NOT DISTINCT**, so a row that predates offers carries `offer_id = NULL` and keeps exactly the old `(org, brand, field_key)` uniqueness), restricted by CHECK to the 7 keys `services`/`dreamOutcome`/`perceivedLikelihood`/`socialProof`/`riskReversal`/`urgency`/`scarcity`. DURABLE (no `expires_at` / TTL). `value` jsonb. Unique `(brand_id, field_key)`. Read as `confirmed` by consumers; the ephemeral auto-extract cache stays on `brand_extracted_fields`
