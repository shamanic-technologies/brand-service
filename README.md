...
| POST / PATCH | `/orgs/brands/:brandId/offers` , `/orgs/brands/:brandId/offers/:offerId` | Create an offer, or rename one. A name the CALLER SUPPLIES is at most **60 characters** with **no word limit** — a customer's own compound name (`Psylium-Swiss-Bio-Drogerien`) is one word to them however many hyphens or spaces it carries — and is unique within the brand (409 rather than a suffixed number). Blank/whitespace-only is 400, and over 60 is a 400 naming the limit: nothing is truncated or rewritten, because the surface that collected the name shows a character counter first. A name brand-service DERIVES for itself stays at **2 words and 20 characters**: the implicit offer a brand-scoped write creates on a brand that has none, and the one the offer migration generates. `201/200 { offer }` |
