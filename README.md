...
| GET | `/orgs/brands/:brandId/brand-profile` | **DEPRECATED** compat shim over `brand_user_fields` (use `/user-fields`). Old shape `{ current: { fields }, versions }`; `fields` = confirmed-overlaid-on-derived + a `valueProposition` alias of `dreamOutcome`. `versions` = `[{ id: null, version: 1, fields, createdAt }]` when confirmed fields exist, else `[]`. Kept until the dashboard migrates. `200` |
| POST | `/orgs/brands/:brandId/brand-profile` | **DEPRECATED** compat shim (use `PUT /user-fields`). Maps legacy `valueProposition`→`dreamOutcome`, keeps ONLY the 7 user-facing keys, upserts them into `brand_user_fields`, silently ignores every other key. Returns the same shape as GET. `201` |
