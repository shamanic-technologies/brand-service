...
| GET | `/orgs/brands/:brandId/sales-funnels` | The sales funnels this brand **declared** it sells through, in catalogue order — `{ funnels: [...] }`. A funnel is one chain from the first signal outreach can buy down to a paid client (`reply_meeting`, `visit_meeting`, `visit_signup`, `visit_form`) and owns its OWN `rates` (only the legs of its chain), `lifetimeRevenueUsd`, `destinationUrl` and `bookingUrl`, plus its `goal` (brand-service wire) + `currentGoal` (runtime token). A value the brand never declared is `null`, never 0. `[]` = declared nothing — read as unknown, NOT as "sells through nothing"; the set is only ever declared, never derived from `brand_sales_economics` (every rate there has a server default, so absence signals nothing). 403 if brand not in caller's org |
...
| DELETE | `/orgs/brands/:brandId/sales-funnels/:funnelKey` | The brand no longer sells through this funnel. Removes the declaration **and its economics** together, so nothing is left behind for a consumer to rank on. Idempotent (undeclaring an undeclared funnel is a 200). Returns the funnels still declared. `200 { funnels }` |
...
| GET | `/internal/brands/:brandId/sales-funnels` | Service-auth read of the funnels a brand **authorizes**, keyed by brandId with NO org context — what campaign-service arbitration ranks over. Same shape as the org read: each funnel carries its `goal` + `currentGoal` and its own rates / lifetime revenue / destinations. `{ funnels: [] }` for a brand that declared nothing — do NOT substitute a plausible set, and do NOT derive one from the stored economics. |
