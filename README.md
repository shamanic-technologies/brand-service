...
| `pnpm exec tsx scripts/backfill-funnel-arrow-rates.ts` | One-time idempotent backfill making every rate a brand states through a NAMED column also readable as the rate of the ARROW it is about (`--dry-run` to preview). Moves no figure, writes nothing for an arrow nobody stated, never overwrites a stated arrow, reversible by `backfilled_at` |
...
| PATCH | `/orgs/brands/:brandId` | Correct the brand identity — PARTIAL: send only the fields you are changing, omitted fields are left as stored. `name` corrects the display name (recorded on `brands.name_set_by_org_at`, after which no automatic name derivation touches the row). `logoUrl` replaces the derived logo with an already-hosted public image (absolute `https://` URL; the file lives in the caller's object storage, this service only stores the URL) and `logoUrl: null` clears it so the derived logo.dev default comes back on the next read; a rejected URL is 400 `INVALID_LOGO_URL`. `url` attaches a website to an existing brand (e.g. a no-website brand whose user later adds their site). Sets url + domain; the next post-cache-expiry field extraction re-sources from the site (rides the existing field cache — no new TTL). **A domain belongs to whoever CHECKED OUT on it** (client-service `GET /internal/brands/:brandId/checkout-status` is the source of truth): if another brand holds the domain but nobody ever checked out on it, the domain is MOVED here and the abandoned holder is left as a no-website brand — and when that holder is the caller's own, its data is merged in and it is dropped from the caller's brand list. Only a paid-on holder is a 409, with a distinct `code`: `DOMAIN_OWNED_BY_YOUR_PAID_BRAND` vs `DOMAIN_OWNED_BY_ANOTHER_ORG` (body also carries `domain` + `conflictingBrandId`). 502 `CHECKOUT_STATUS_UNAVAILABLE` if client-service can't answer (never assumed unpaid); 403 if not in caller's org |
...
| GET | `/orgs/brands/:brandId/sales-funnels` | Every funnel this org has configured on this brand — `{ funnels }`, ACTIVE and inactive alike, each with `active`. A funnel is one chain from the event that STARTS it down to the SALE (`sales_meetings_from_conversation`, `sales_meetings_from_website`, `website_purchases`, `form_magnet`, `sales_from_conversation`, `sales_meetings_from_ads`, `lead_forms_from_ads`, `sales_from_website`) and owns its OWN `rates` (only the legs of its chain), `lifetimeRevenueUsd`, `destinationUrl` and `bookingUrl`. Every funnel states `startEvent` (`conversation_reply` / `website_visit` / `ad_click` / `meeting_booked` / `lead_form_submitted` — an ad that delivers a booked meeting or a filled platform form starts on the step the channel DELIVERS, not on the click before it) — how a consumer decides which acquisition channels can feed it — and `milestoneStep` + `milestoneStepIndex`, the step the funnel is NAMED after: the moment that tells a brand it is working, and what a channel's minimum budget is priced against (one month must pay for at least one). The milestone is always a member of `steps`, so a consumer reads it instead of hardcoding a mapping. It carries **no goal**: that vocabulary is retired because both meeting funnels collapsed onto one `meetingBooked`, so no consumer could price them apart. The pre-retirement keys (`reply_meeting`, `visit_meeting`, `visit_signup`, `visit_form`) are accepted on WRITE forever and resolved to the canonical ones, and so is a pre-reshape RATE word: `sales_from_website` reads `Website visit -> Purchase -> Paid client` (milestone `Purchase`, legs `visitToPurchasePct` + `purchaseToPaidClientPct`), and the `visitToClosePct` that priced its single leg before the purchase became its own rung resolves onto `visitToPurchasePct` — the arrow a caller sending it is stating. Scoped to that funnel: the same word against `website_purchases` names no arrow it prices and is still refused. A value the brand never declared is `null`, never 0. An inactive funnel keeps every number on it, which is what makes switching it back on return what the user entered. An EMPTY list means the org has **never answered** — a gap to surface, never "sells through nothing", which is unreachable because an org that answered always keeps one funnel on. The set is only ever stated, never derived from `brand_sales_economics` (every rate there has a server default, so absence signals nothing). 403 if brand not in caller's org |
...
| GET | `/orgs/brands/:brandId/sales-rep` | The ONE person to reach when a sales interest lands on this brand, and the two facts we hold about them: the address to COPY them on the prospect's own thread, and the number to RING. A brand that never stated a rep reads `{ salesRepEmail: null, salesRepPhone: null }` — "nobody to reach", a first-class answer, not a 404 (3 of 188 brands have a rep, so absence is the ordinary case). Either field is independently `null`: a rep stated before the email existed carries a phone and no address, and a rep meant to be copied but never rung carries an address and no number. Per-brand config keyed on `(org, brand)`; 403 if brand not in caller's org. `200 { salesRepEmail, salesRepPhone }` |
| PUT | `/orgs/brands/:brandId/sales-rep` | State (or change) the whole rep in one write. Body `{ salesRepEmail, salesRepPhone? }`. **A PHONE REQUIRES AN EMAIL** — a body carrying a number and no address is refused **400** with a sentence a person can act on, because the rep is copied on the prospect's reply and a phone alone cannot do that; the dashboard renders that sentence verbatim and implements no validation of its own. **An EMAIL WITH NO PHONE is legal and useful** (copied, never rung — what the AI meeting-booking channel needs). The write replaces the WHOLE rep, so omitting `salesRepPhone` (or sending `null`) CLEARS a number that was there: two facts about one person, never two half-writes. The phone accepts any typed format carrying a country code (`+`/`00`, 8-15 digits) and stores strict E.164; the email is trimmed and otherwise stored exactly as typed, must be ONE bare address (the `Name <address>` form, several addresses, or anything unparseable → 400). A body stating neither fact → 400 naming DELETE. Idempotent upsert; 403 if brand not in caller's org. `200 { salesRepEmail, salesRepPhone }` |
| DELETE | `/orgs/brands/:brandId/sales-rep` | Remove the rep: the row is deleted and the brand goes back to "nobody to reach". Both facts go at once — they describe one person, and there is no such thing as half a rep. Idempotent — removing a rep that was never stated is a 200, not a 404. `200 { salesRepEmail: null, salesRepPhone: null }` |
| GET | `/orgs/brands/:brandId/sales-rep-phone` | **TRANSITIONAL.** The one number to ring when a sales interest lands on this brand, or `null` when the brand has no number ("nobody to ring" — a first-class answer, not an error). The response ALSO carries `salesRepEmail`, additively, so a service already calling this route (instantly-service does, with the service key plus `x-org-id`) reads the address without a new call or a deploy-ordering gate. `GET /sales-rep` is the same answer under the rep name. Per-brand config keyed on `(org, brand)`; 403 if brand not in caller's org. `200 { salesRepEmail, salesRepPhone }` |
| PUT | `/orgs/brands/:brandId/sales-rep-phone` | **TRANSITIONAL — the ONE place a phone may be written without an email**, kept unchanged so the dashboard's existing phone-only path keeps working while the rep contract ships and nothing has to deploy in a particular order; retired once its consumer has moved to `PUT /sales-rep`. Any stored email is PRESERVED. State (or change) the number to ring when a prospect replies interested — the rep on it is phoned within the minute. BRAND grain (one number covers every offer, funnel and channel), per-brand config mirroring click-destination scoping. Body `{ salesRepPhone }` accepts any typed format carrying a country code (`+` or `00` prefix, 8-15 digits) and stores strict E.164; a national number with no country code, or anything unparseable, → 400 (nothing is inferred — the value goes straight to a telephony provider). Idempotent upsert; 403 if brand not in caller's org. Read back here or via the `salesRepPhone` field on the INTERNAL brand read. `200 { salesRepPhone }` |
| DELETE | `/orgs/brands/:brandId/sales-rep-phone` | **TRANSITIONAL.** Remove the NUMBER — exactly what this route has always meant, and no more. A rep who also has an email keeps their row with `salesRepPhone: null`: still somebody to copy, just nobody to ring. Only a rep with nothing left is removed outright (a row carrying neither fact is not a rep). It deliberately does NOT wipe an address stated through `/sales-rep`. Idempotent — removing a number that was never stated is a 200, not a 404. `200 { salesRepEmail, salesRepPhone: null }` |
| POST | `/orgs/brands/:brandId/offers/:offerId/image` | (Re)generate the picture standing for ONE offer — an OBJECT or EMBLEM symbolising what is sold, never a face — and store the hosted URL on it. Regenerating REPLACES what was there (one image per offer). The prompt is derived from the offer's own descriptors (name + confirmed `services` / `dreamOutcome`; name alone when it has stated none) and its BACKGROUND COLOUR is picked deterministically from a fixed palette seeded on the offer id, so the colour survives every regeneration and two offers of one brand differ. Optional body `{ prompt }` steers the image. Cost + affordability owned by chat-service (the terminal image caller) and billed to the requesting ORG — brand-service declares none; its 402 propagates as 402, generation failure is a loud 502 and nothing is stored. `200 { offer }` with `offer.imageUrl` set |
...
| GET | `/orgs/brands/:brandId/user-fields` | The 8 user-facing "confirmed" fields: `{ fields: { <key>: { value, provenance } } }` for services, dreamOutcome, perceivedLikelihood, socialProof, riskReversal, urgency, scarcity, targetAudience. `confirmed` = user-validated value; `suggested` = most-recent NON-EXPIRED auto-extract prefill (or null). Does NOT trigger extraction. 403 if brand not in caller's org |
...
| POST | `/internal/brands/domain-claimed` | Does any organisation already claim the brand behind this website? Body `{ domain }` (a bare domain or a full URL, normalized server-side; body-carried so it stays out of access logs), answers `{ domain, claimed }`. The boolean is the **whole** answer — never which org, how many, or anything identifying them: the caller acts for somebody with **no account** (the dashboard's signed-out onboarding, which spends real money against a throwaway org and reads a brand's scraped site and extracted fields — neither of which carries an org column). **Creates nothing**: no brand row, no claim, no scrape, no LLM call, no cost, so a stranger typing a URL into a landing page cannot make us do work. A domain nobody has ever sent us is the common case and answers `false`; so does a brand row no org claims (genuinely unclaimed). **Only a REAL organisation counts as a claimant** — the signed-out flow mints an anonymous org before anything else exists and creates the brand against it, and nearly every such walk is abandoned, so a brand whose every owner is an unclaimed anonymous org is **not** claimed and its domain is free. client-service owns that fact (`POST /internal/orgs/real`) and is asked for it; an anonymous org that has since been claimed is real and still locks the domain. If that answer cannot be obtained this **502s** — never a defaulted verdict either way. One indexed lookup on the unique `domain` index, plus that one question. Internal only; not on the public router. |
...
- `brand_offers` — one row per `(org_id, brand_id, name)`: one distinct thing a brand sells, and the level every funnel, arrow rate and value proposition hangs off. THERE IS NO PRIMARY OFFER. The NAME is split by who wrote it: a name a CALLER SUPPLIES (`POST /orgs/brands/:brandId/offers`, `PATCH .../offers/:offerId`) is accepted up to **60 characters with no word limit** — a customer's own compound proposition (`Psylium-Swiss-Bio-Drogerien`) is one name to them whatever a word counter says — while a name the service DERIVES for itself (the implicit offer created by a legacy brand-scoped write, and anything the one-time migration generates) stays at **at most 2 words and at most 20 characters**, because the shortening only ever drops trailing words and needs a target to cut to. Both rules live in `src/lib/offer-name.ts` (`offerNameProblem` vs `derivedOfferNameProblem`); the 60-character ceiling is also a table CHECK, so a script writing around the service cannot store a name no surface can render. A supplied name is never truncated: over the limit is a 400 naming the limit, and the name is UNIQUE within the brand (409 `OFFER_NAME_TAKEN`, never suffixed with a number). `migrated_at` is provenance for the one-time migration; `image_url` is the offer's own emblem, nullable and never defaulted
...
- `brand_sales_funnel_arrow_rates` — one row per `(offer_id, funnel_key, from_step, to_step)`: the rate a brand states for ONE ARROW of one of its funnels, identified by the two STEPS it connects instead of by a name from a closed list. That is what lets a brand price an arrow brand-service does not know in advance — funnels are gaining steps (a phone call between a positive reply and a booked meeting is the first), and under the named-column model each new step costs a migration plus a rename wave through every consumer. `rate_pct` numeric(7,4) NOT NULL; clearing a statement DELETES the row, so "not stated" is one state. ADDITIVE: the named rate columns on `brand_sales_funnels` are untouched and still answer exactly what they answer today. **PRECEDENCE**, where both describe the same arrow: the STATED ARROW WINS, the named rate is the fallback (`provenance: stated_arrow` / `named_rate` / `unstated` on every arrow of every funnel read). `offer_id` is NOT NULL here, unlike on `brand_sales_funnels` — the table is new, so it has no rows predating the offer migration. `backfilled_at` (timestamptz, nullable) records an arrow COPIED from the named column describing the same arrow by the one-time backfill (`scripts/backfill-funnel-arrow-rates.ts`) rather than stated arrow-first by a customer — provenance only, read by nothing, and what makes that run reversible by an exact predicate. Its idempotency marker is `brand_sales_funnels.arrow_rates_backfilled_at`, which is also what stops a re-run resurrecting an arrow the customer deleted
...
- `brand_sales_rep_phones` — one row per `(org_id, brand_id)`: the ONE SALES REP for the brand and the two facts we hold about them — `email` text (the address COPIED on the prospect's own thread, by the service forwarding a positive reply to the agency inbox and by the AI meeting-booking channel's one-to-one reply) and `phone` text in strict E.164 (the number RUNG within the minute). **The table name is HISTORICAL**: it was born holding a phone alone (migration `0060`) and `email` arrived in `0069`; it is not renamed because `brandMergeService` addresses it by literal string and every drizzle snapshot carries the old name. One rep, one row, both facts — there is no second table and no second home for the same person. BOTH columns are nullable and `sales_rep_has_a_fact` CHECKs that at least one is set: a row carrying neither is not a rep, and clearing the rep DELETES the row, so row presence = "there is a rep" and each column's presence = "we hold that fact". **The product rule — a phone requires an email — is enforced at the WRITE, never by the database**: 3 production rows carry a phone and no email (we were never told those reps' addresses and nothing invents one), so a CHECK would make them unwritable and recast a true record of a fact we do not have as a constraint violation; they keep being rung and are simply never copied. BRAND grain deliberately, not campaign grain: a campaign is (offer x funnel x channel), so a brand running four channels on one offer would retype one person four times, and a brand with no campaign yet could state nothing at all. Per-brand config (mirrors `brand_click_destinations` scoping); an unset brand has no row and the INTERNAL brand read returns `salesRepEmail: null` + `salesRepPhone: null` — never an empty string, never defaulted or inferred from the user row's phone, the WhatsApp link, or the org owner's account email (a guess about who the rep is, and being wrong mails a client's prospect thread to the wrong person). Both fields are served on the internal brand reads only, under ONE flag and ONE resolution so the two can never diverge into one being reachable where the other is not (org-scoped by `x-org-id`; where no org is named, it answers only when a single org has stated a rep), and both are absent from the public brand read and the share-token resolve
...
- `brand_user_fields` — the user-facing "confirmed" layer: one row per `(org_id, brand_id, field_key)`, restricted by CHECK to the 8 keys `services`/`dreamOutcome`/`perceivedLikelihood`/`socialProof`/`riskReversal`/`urgency`/`scarcity`/`targetAudience` (the last admitted 2026-09-19 for the onboarding's "Who do you sell to?" answer). DURABLE (no `expires_at` / TTL). `value` jsonb. Unique `(brand_id, field_key)`. Read as `confirmed` by consumers; the ephemeral auto-extract cache stays on `brand_extracted_fields`
