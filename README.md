...
| PUT | `/orgs/brands/:brandId/click-destination` | Set the brand's click-destination URL (the page outreach clicks land on; per-brand config, mirrors sales-economics scoping). Body `{ clickDestinationUrl }` must be an absolute http(s) URL (non-http(s)/unparseable → 400). Idempotent upsert; 403 if brand not in caller's org. Read back via the `clickDestinationUrl` field on the brand read (`null` when unset). `200 { clickDestinationUrl }` |
